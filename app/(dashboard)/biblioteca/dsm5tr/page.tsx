...
import { mockDSM5TR } from "@/lib/mock-data";
...
function DSM5TR() {
  ...
  const filtered = mockDSM5TR.filter(
    (c) => c.nombre.toLowerCase().includes(q) || c.codigo.toLowerCase().includes(q)
  );

  return (
    <div className="flex flex-col flex-1 min-h-0">
      <SectionHeader title="Inicio - Biblioteca digital - DSM-5TR" />
      ...
      <div className="flex-1 overflow-auto p-5 min-h-0">
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4 flex items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Buscar por nombre o código (ej. F32, depresivo)"
        ...
        <p className="mt-4 text-xs text-white/70">
          * Extracto orientativo. El DSM-5TR completo está disponible en la fuente oficial de la APA.
        </p>
      </div>
    </div>
  );
...
export default function DSM5TRPage() {
  return (
